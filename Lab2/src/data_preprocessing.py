"""
Stage 2: Data Preprocessing (Regression Version)
-----------------------------
Reads the raw CSV, cleans column names, checks/handles missing values and
duplicates, renames the target column 'medv' to 'target', and writes a processed CSV.
...
def clean_data(df: pd.DataFrame) -> pd.DataFrame:
    ...
    # Rename medv to target
    if "medv" in df.columns:
        df = df.rename(columns={"medv": "target"})
        print("[data_preprocessing] Renamed 'medv' column to 'target'")
    else:
        print("[data_preprocessing] Warning: 'medv' column not found in raw data")
